...
class TwoLayerNet(object):
	...
	def loss(self, X, y=None):
		...
		y2, cache2 = affine_forward(z1, self.params['W2'], self.params['b2'])
		scores = y2		 
		############################################################################
		#															END OF YOUR CODE														 #
		############################################################################
		...
		############################################################################
		loss, dout = softmax_loss(y2, y)
		loss += self.reg * 0.5 * (np.sum(np.power(self.params['W1'], 2)) + np.sum(np.power(self.params['W2'], 2)))
...
class FullyConnectedNet(object):
	...
	def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
		...
		for i in range(1, self.num_layers+1):
			self.params["W" + str(i)] = np.random.normal(0, scale=weight_scale, size=(hidden_dims[i-1], hidden_dims[i])) 
	...
	def loss(self, X, y=None):
		...
		for i in range(1, self.num_layers):
			if self.use_dropout:
				a, cache[i] = affine_relu_dropout_forward(a, self.params['W'+str(i)], 
																									self.params['b'+str(i)], 
																									self.dropout_param)
			elif self.use_batchnorm:
				a, cache[i] = affine_relu_bn_forward(a, self.params['W'+str(i)], 
																							self.params['b'+str(i)], 
																							self.params['gamma'+str(i)],
																							self.params['beta'+str(i)],
																							self.bn_params[i-1])
			else:
				a, cache[i] = affine_relu_forward(a, self.params['W'+str(i)], self.params['b'+str(i)])
		scores, cache[self.num_layers] = affine_forward(a, self.params['W'+str(self.num_layers)], 
		...
		S = 0
		for i in range(1, self.num_layers+1):
			S += np.sum(np.power(self.params['W'+str(i)], 2))
		loss += self.reg * 0.5 * S

		dx, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dout, cache[-1])
		for i in range(self.num_layers-1, 0, -1):
			if self.use_dropout:
				dx, grads['W'+str(i)], grads['b'+str(i)] = affine_relu_dropout_backward(dx, cache[i])
			elif self.use_batchnorm:
				dx, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_relu_bn_backward(dx, cache[i])
			else:
